  ...
  class Relation
    ...
    def initialize(model, term = "*", base_scope: model.all, fields:, misspellings:, where: {}, order: nil, limit: nil, offset: nil, page: nil, per_page: nil, padding: nil, match: :word, operator: "and", load: true, total_entries: nil, countless: false, keyset: false, after: nil, aggs: nil, smart_aggs: true, includes: nil, model_includes: nil, scope_results: nil, select: nil, exclude: nil, highlight: nil, boost_by: nil, boost_where: nil, boost: nil, boost_by_recency: nil, conversions: nil, conversions_v2: nil, conversions_term: nil, block: nil, tinql: nil)
      ...
      @options = {
        ...
        select: select, exclude: exclude, highlight: highlight, boost_by: boost_by, boost_where: boost_where, boost: boost, boost_by_recency: boost_by_recency, conversions: conversions, conversions_v2: conversions_v2, conversions_term: conversions_term, block: block, tinql: tinql,
      }
      query
    end
    ...
    def merge(scope)
      unless scope.is_a?(ActiveRecord::Relation) && scope.klass == @model
        raise ArgumentError, "merge requires an Active Record relation for #{@model.name}"
      end

      copy = clone
      copy.instance_variable_set(:@base_scope, @base_scope.merge(scope))
      copy
    end

    def only(*keys)
      options = @options.slice(*keys) #: relation_option_subset
      @base_scope.scoping { @model.tinkick_search(@term, **options) }
    end
    ...
    def pluck(*keys)
      ...
      Tinkick.warn(@model, "Tinkick: load: false is supported for Searchkick compatibility. Migrate to model results when possible; both modes query PostgreSQL through Active Record.")
    ...
    def query
      ...
      @query ||= Query.new(@model, @term,
        ...
        countless: @options[:countless], keyset: @options[:keyset], after: @options[:after], aggs: @options[:aggs], smart_aggs: @options[:smart_aggs], exclude: @options[:exclude], boost_by: @options[:boost_by], boost_where: @options[:boost_where], boost: @options[:boost], boost_by_recency: @options[:boost_by_recency], conversions: @options[:conversions], conversions_v2: @options[:conversions_v2], conversions_term: @options[:conversions_term], block: @options[:block], tinql: @options[:tinql])
    end

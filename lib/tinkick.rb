...
module Tinkick
  class << self
    def search(term = "*", model:, fields: nil, misspellings: true, where: {}, order: nil,
      ...
      after: nil, aggs: nil, smart_aggs: true, includes: nil, model_includes: nil, scope_results: nil)
      model.tinkick_search(term, fields: fields, misspellings: misspellings, where: where, order: order,
        ...
        after: after, aggs: aggs, smart_aggs: smart_aggs, includes: includes, model_includes: model_includes, scope_results: scope_results)
    end
  end
end

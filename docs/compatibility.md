...
| Full-field highlights/custom tags | `tin.highlight` with explicit tags and optional query. [Highlighting](https://planetscale.com/docs/postgres/search/highlighting) | Public default-analysis highlighting, custom tags, HTML encoding, page batches, snippets and result helpers are tested. Refined fuzzy modes verify eligible page tokens; SQL modes verify and mark the complete field. Default encoding preserves source HTML; returned strings are not marked HTML-safe. Custom analysis remains adapter work. |

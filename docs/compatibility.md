...
| `search_data` / `tinkick_search_data` | Intentional change: the prefixed hook validates field names against model columns without serializing values. `search_data` is a fallback only without Searchkick in the application bundle; during coexistence it belongs exclusively to Searchkick. Without an applicable hook, use model columns. Derived data belongs in stored or generated columns added through Rails migrations; missing columns raise migration guidance. |
...
| Ranking | Native field boosts and SQL field weights are tested; [captured plans](query-plans.md#native-and-sql-field-weights) show native top-k versus SQL grouping/sorting. Numeric boosts over columns, numeric arrays and JSONB paths, `boost_where`, typed-column recency, and application-owned JSONB conversions are implemented. [Recency plans](recency-plans.md) and [conversion plans](conversion-plans.md) show SQL scoring costs. Model boosts remain dependent on multi-model search. |
| Aggregations | Terms, numeric/date ranges and histograms, metrics, per-aggregation filters, limits/order, minimum counts, smart facets, and numeric/date bounds are implemented in SQL. Date buckets use native PostgreSQL date_trunc/date_bin and generate_series with fixed offsets or IANA zones. Date math and Java date formats are Elasticsearch-specific and rejected; compute boundaries and labels in the application. Elasticsearch nested/pipeline request bodies and Java numeric formatting are excluded; use SQL grouping/subqueries and application formatting. Terms include/exclude use native SQL value comparisons or PostgreSQL regex strings. Terms, metrics, ranges and histograms support missing defaults with native COALESCE, including one fallback per empty/all-null array document. DATE defaults preserve calendar dates; timestamp defaults use normalized instants. Scalar JSONB aggregation paths remain follow-up work. |
...
| Beyond lexical search | Bounded result lists can be combined with Tinkick::Reranking.rrf. Suggestions, similar items, geospatial, KNN, and semantic/hybrid retrieval remain follow-up work. No absence claim follows from an unimplemented adapter. TIN's overview describes pgvector composition for hybrid retrieval. |

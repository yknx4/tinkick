...
| String filters | `like`, `ilike`, prefix, Ruby Regexp translation, and whole-value raw-string `regexp` are implemented. Lucene optional syntax uses a warned recursive SQL character walk; ordinary patterns use PostgreSQL regex. Neither requires an optional extension. |

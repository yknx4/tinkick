...
| Ranking | Native field boosts and SQL field weights are tested; [captured plans](query-plans.md#native-and-sql-field-weights) show native top-k versus SQL grouping/sorting. Public numeric boosts over table columns and numeric arrays are tested, including modifiers and missing values. JSON numeric paths, `boost_where`, recency, conversions and model boosts remain adapter work. |

...
3. **Highlight public surface and portable response.** Cached hits/with_hit use durable model PK, PostgreSQL table identity, native score, and selected source snapshots; pre-scope hits and visible record pairs are tested. Cached response includes page timing, those hits, ordinary exact totals and requested aggregations; countless/keyset omit unknown totals without COUNT. Public highlight keyword/fluent options, per-field configuration, highlights/with_highlights, record search_highlights, raw highlighted_* values, custom tags, HTML encoding and Unicode-safe snippets are implemented for default-analysis native modes. Refined fuzzy highlights use one page-bounded eligibility batch before native marking, with a cost warning. Custom index analysis and SQL whole-field highlighting still require adapter work; no broad fuzzy candidate or wrong analyzer is silently used for spans.
...
5. **Ranking and remaining aggregation shapes.** Field ^boost, boost/boost_by numeric modifiers, boost_where/demotion, boost_by_recency, conversions/conversions_v2/conversions_term. Numeric histograms with bounds, calendar/fixed date histograms with fixed time zones and IANA calendar day-or-larger units, compound relevance ordering, and numeric custom date formats are implemented. Remaining: IANA subday/fixed intervals, offset/bounded date histograms, advanced date/numeric formats, nested aggregation shapes and options. SQL can supply the machinery; exact formulas, NULL behavior, array semantics, cost warnings and real plans are required. Model/indices boosts wait for multi-model identity; geographic distance boosts wait for geo integration.

...
5. **Ranking and remaining aggregation shapes.** Field ^boost, boost/boost_by numeric modifiers, boost_where/demotion, boost_by_recency, conversions/conversions_v2/conversions_term. Numeric histograms with bounds, calendar/fixed date histograms with fixed time zones, compound relevance ordering, and numeric custom date formats are implemented. Remaining: IANA timezone/offset/bounded date histograms, advanced date/numeric formats, nested aggregation shapes and options. SQL can supply the machinery; exact formulas, NULL behavior, array semantics, cost warnings and real plans are required. Model/indices boosts wait for multi-model identity; geographic distance boosts wait for geo integration.

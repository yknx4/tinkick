...
2. **Projection and query option operations.** Top-level and nested JSON select/reselect source filtering, Enumerable block selection, hidden cursor-key preservation, only/except on options, and explicit _score/compound ordering are implemented. Remaining: wildcard search-field expansion. Model results retain complete attributes according to pinned Searchkick behavior.

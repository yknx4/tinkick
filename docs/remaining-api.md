...
3. **Highlight public surface and portable response.** Cached hits/with_hit use durable model PK, PostgreSQL table identity, native score, and selected source snapshots; pre-scope hits and visible record pairs are tested. Cached response includes page timing, those hits, ordinary exact totals and requested aggregations; countless/keyset omit unknown totals without COUNT. Wire the existing tested Highlighter into highlight keyword/fluent options, per-field configuration, highlights/with_highlights, record search_highlights, raw highlighted_* values, custom tags, HTML encoding, and snippets/fragment_size. Default-analysis full-field native support is proven; custom index analysis, snippet compatibility and alternate SQL match highlighting still require adapter work.

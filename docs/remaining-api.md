...
3. **Highlight public surface and portable response.** Cached hits/with_hit use durable model PK, PostgreSQL table identity, native score, and selected source snapshots; pre-scope hits and visible record pairs are tested. Cached response includes page timing, those hits, ordinary exact totals and requested aggregations; countless/keyset omit unknown totals without COUNT. Public highlight keyword/fluent options, per-field configuration, highlights/with_highlights, record search_highlights, raw highlighted_* values, custom tags, HTML encoding and Unicode-safe snippets are implemented for default-analysis native modes. Refined fuzzy, custom index analysis and SQL whole-field highlighting still require adapter work; no broad fuzzy candidate or wrong analyzer is silently used for spans.

...
class DateHistogramFormatsTest < TinkickIntegrationTest
  def test_public_month_format_changes_labels_without_changing_keys_or_counts
    ...
    buckets = search(calendar_interval: :month, format: "yyyy/MM||epoch_millis").aggs.fetch("events").fetch("buckets")

    assert_equal ["2026/01", "2026/02", "2026/03"], buckets.map { |bucket| bucket.fetch("key_as_string") }
  ...
  def test_fixed_interval_format_uses_the_requested_offset_clock
    ...
    buckets = search(fixed_interval: "1h", time_zone: "+01:30", format: "yyyy/MM/dd HH:mm XXX").aggs.fetch("events").fetch("buckets")

    assert_equal ["1970/01/01 02:00 +01:30", "1970/01/01 03:00 +01:30"], buckets.map { |bucket| bucket.fetch("key_as_string") }
  ...
  def test_iana_gap_and_keyed_format_use_the_resolved_first_valid_boundary
    ...
    buckets = search(calendar_interval: :day, time_zone: "America/Havana", format: "yyyy/MM/dd HH:mm XXX", keyed: true)
      .aggs.fetch("events").fetch("buckets")

    assert_equal ["2026/03/07 00:00 -05:00", "2026/03/08 01:00 -04:00"], buckets.keys
  ...
  def test_epoch_millis_keys_stay_utc_when_the_calendar_uses_an_iana_zone
    ...
    buckets = search(calendar_interval: :day, time_zone: "America/New_York", format: "epoch_millis||strict_date_optional_time", keyed: true)
      .aggs.fetch("events").fetch("buckets")
  ...
  def test_format_requires_an_inner_string_and_valid_supported_pattern
    ["", nil, false, 123, {}, "yyyy 'unfinished", "yyyy-MM-dd z"].each do |format|

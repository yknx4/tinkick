...
class IanaSubdayHistogramsTest < TinkickIntegrationTest
  # PostgreSQL date_trunc owns ambiguity and historical timezone behavior.
  CASES = [
    [:new_york_first_hour, :hour, "America/New_York", "2026-11-01T05:30:00Z"],
    [:new_york_second_hour, :hour, "America/New_York", "2026-11-01T06:30:00Z"],
    [:new_york_spring_hour, :hour, "America/New_York", "2026-03-08T07:30:00Z"],
    [:lord_howe_fall_hour, :hour, "Australia/Lord_Howe", "2026-04-04T15:15:00Z"],
    [:lord_howe_spring_hour, :hour, "Australia/Lord_Howe", "2026-10-03T15:45:00Z"],
    [:havana_first_hour, :hour, "America/Havana", "2026-11-01T04:30:00Z"],
    [:havana_second_hour, :hour, "America/Havana", "2026-11-01T05:30:00Z"],
    [:havana_gap_hour, :hour, "America/Havana", "2026-03-08T05:30:00Z"],
    [:moncton_historic_hour, :hour, "America/Moncton", "2003-10-26T03:43:35.079Z"],
    [:chatham_fall_hour, :hour, "Pacific/Chatham", "2015-04-04T14:15:00Z"],
    [:kathmandu_historic_hour, :hour, "Asia/Kathmandu", "1985-12-31T18:35:00Z"],
    [:singapore_historic_hour, :hour, "Asia/Singapore", "1945-09-11T15:15:00Z"],
    [:casey_historic_hour, :hour, "Antarctica/Casey", "2010-03-04T15:30:00Z"],
    [:athens_historic_hour, :hour, "Europe/Athens", "1916-07-27T22:26:08Z"],
    [:amsterdam_historic_second, :second, "Europe/Amsterdam", "1937-06-30T22:40:30.123Z"],
  ].freeze

  CASES.each do |name, unit, zone, input|
    define_method("test_#{name}") do
      record = create_value(Time.iso8601(input), 1)
      expected = IanaSubdayValue.where(id: record.id).pick(Arel.sql(
        "(EXTRACT(EPOCH FROM date_trunc(?, recorded_at AT TIME ZONE 'UTC', ?)) * 1000)::bigint", unit.to_s, zone))
  ...
  def test_historical_amsterdam_uses_postgresql_offsets_for_keys_and_labels
    ...
    assert_equal [{ "key" => Time.utc(1937, 6, 30, 22, 40).to_i * 1_000,
                    "key_as_string" => "1937-06-30T23:59:32.000+01:19", "doc_count" => 1 }], buckets
  end
  ...
  def test_lord_howe_native_series_retains_populated_truncation_boundaries
    ...
    assert_equal [Time.utc(2026, 4, 4, 14), Time.utc(2026, 4, 4, 15), Time.utc(2026, 4, 4, 16), Time.utc(2026, 4, 4, 17), Time.utc(2026, 4, 4, 17, 30)]
      .map { |instant| instant.to_i * 1_000 }, buckets.map { |bucket| bucket.fetch("key") }
    assert_equal [1, 0, 0, 0, 1], buckets.map { |bucket| bucket.fetch("doc_count") }
  end
  ...
  def test_offset_order_format_and_minimum_count_apply_after_utc_rounding
    ...
    options = { calendar_interval: :hour, time_zone: "America/New_York", offset: "30m", order: { _key: :desc }, min_doc_count: 1 }
    buckets = search(**options)

    assert_equal ["2026-11-01T01:30:00.000-05:00", "2026-11-01T01:30:00.000-04:00"], buckets.map { |bucket| bucket.fetch("key_as_string") }
